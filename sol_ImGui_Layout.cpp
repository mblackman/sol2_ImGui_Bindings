...
void PushStyleVar(int idx, float valX, float valY) { ImGui::PushStyleVar(idx, {valX, valY}); }
...
int GetColorU32(float colR, float colG, float colB, float colA) { return ImGui::GetColorU32({colR, colG, colB, colA}); }
  ...
  // No p_open
  return ImGui::BeginTabItem(label.c_str(), nullptr, 0);
    }
...
#else
void DockSpace(unsigned int) { /* UNSUPPORTED: Docking not enabled/available */ }

void DockSpace(unsigned int, float, float) { /* UNSUPPORTED: Docking not enabled/available */ }

void DockSpace(unsigned int, float, float, int) { /* UNSUPPORTED: Docking not enabled/available */ }

unsigned int DockSpaceOverViewport() { return 0; /* UNSUPPORTED: Docking not enabled/available */ }

void SetNextWindowDockID(unsigned int) { /* UNSUPPORTED: Docking not enabled/available */ }

void SetNextWindowDockID(unsigned int, int) { /* UNSUPPORTED: Docking not enabled/available */ }

void SetNextWindowClass() { /* UNSUPPORTED: Docking not enabled/available */ }

unsigned int GetWindowDockID() { return 0; /* UNSUPPORTED: Docking not enabled/available */ }

bool IsWindowDocked() { return false; /* UNSUPPORTED: Docking not enabled/available */ }
#endif
...
#endif
  imGuiTable.set_function("PopStyleColor",
                          sol::overload(sol::resolve<void()>(PopStyleColor), sol::resolve<void(int)>(PopStyleColor)));
  imGuiTable.set_function("PushStyleVar", sol::overload(sol::resolve<void(int, float)>(PushStyleVar),
                                                        sol::resolve<void(int, float, float)>(PushStyleVar)));
  imGuiTable.set_function("PopStyleVar",
                          sol::overload(sol::resolve<void()>(PopStyleVar), sol::resolve<void(int)>(PopStyleVar)));
...
#pragma region Parameters stacks (current window)
  ...
  imGuiTable.set_function("PushTextWrapPos", sol::overload(sol::resolve<void()>(PushTextWrapPos),
...
#pragma endregion Parameters stacks(current window)
  ...
  imGuiTable.set_function("SameLine", sol::overload(sol::resolve<void()>(SameLine), sol::resolve<void(float)>(SameLine),
  ...
  imGuiTable.set_function("Indent", sol::overload(sol::resolve<void()>(Indent), sol::resolve<void(float)>(Indent)));
  imGuiTable.set_function("Unindent",
                          sol::overload(sol::resolve<void()>(Unindent), sol::resolve<void(float)>(Unindent)));
  ...
  imGuiTable.set_function("BeginTabBar", sol::overload(sol::resolve<bool(const std::string &)>(BeginTabBar),
                                                       sol::resolve<bool(const std::string &, int)>(BeginTabBar)));
  imGuiTable.set_function("EndTabBar", EndTabBar);
  imGuiTable.set_function(
      "BeginTabItem",
      sol::overload(sol::resolve<bool(const std::string &)>(BeginTabItem),       // No p_open
  ...
  imGuiTable.set_function("DockSpace", sol::overload(sol::resolve<void(unsigned int)>(DockSpace),
                                                     sol::resolve<void(unsigned int, float, float)>(DockSpace),
                                                     sol::resolve<void(unsigned int, float, float, int)>(DockSpace)));
  imGuiTable.set_function(
      "SetNextWindowDockID",
      sol::overload(sol::resolve<void(unsigned int)>(SetNextWindowDockID),  // unsigned int for ImGuiID
  ...
  imGuiTable.set_function("IsItemHovered",
  ...
  imGuiTable.set_function("IsItemClicked",
  ...
  imGuiTable.set_function("IsRectVisible",
