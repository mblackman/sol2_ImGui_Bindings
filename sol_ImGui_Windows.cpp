...
std::tuple<bool, bool> Begin(const std::string &name, bool open) {
  ...
  bool should_draw = ImGui::Begin(name.c_str(), open ? &p_open_value : nullptr);
  return std::make_tuple(p_open_value, should_draw);
}

std::tuple<bool, bool> Begin(const std::string &name, bool open, int flags) {
  bool p_open_value = open;
  bool should_draw = ImGui::Begin(name.c_str(), open ? &p_open_value : nullptr, flags);
  return std::make_tuple(p_open_value, should_draw);
}

void End() { ImGui::End(); }

bool BeginChild(const std::string &name) { return ImGui::BeginChild(name.c_str(), {0, 0}, ImGuiChildFlags_None, 0); }
...
bool BeginChild(const std::string &name, float sizeX, float sizeY, bool border) {
  return ImGui::BeginChild(name.c_str(), {sizeX, sizeY}, border ? ImGuiChildFlags_Borders : ImGuiChildFlags_None, 0);
}
...
void SetNextWindowPos(float posX, float posY, int cond) { ImGui::SetNextWindowPos({posX, posY}, cond); }
...
void SetNextWindowSize(float sizeX, float sizeY, int cond) { ImGui::SetNextWindowSize({sizeX, sizeY}, cond); }
...
void SetNextWindowCollapsed(bool collapsed, int cond) { ImGui::SetNextWindowCollapsed(collapsed, cond); }
...
void SetWindowPos(float posX, float posY, int cond) { ImGui::SetWindowPos({posX, posY}, cond); }

void SetWindowSize(float sizeX, float sizeY) { ImGui::SetWindowSize({sizeX, sizeY}); }

void SetWindowSize(float sizeX, float sizeY, int cond) { ImGui::SetWindowSize({sizeX, sizeY}, cond); }

void SetWindowCollapsed(bool collapsed) { ImGui::SetWindowCollapsed(collapsed); }

void SetWindowCollapsed(bool collapsed, int cond) { ImGui::SetWindowCollapsed(collapsed, cond); }
...
void Init_ChildWindows(sol::table &imGuiTable) {
  imGuiTable.set_function("BeginChild",
...
void Init_WindowUtilities(sol::table &imGuiTable) {
  ...
  imGuiTable.set_function("IsWindowFocused", sol::overload(sol::resolve<bool()>(IsWindowFocused),
                                                           sol::resolve<bool(int)>(IsWindowFocused)));
  imGuiTable.set_function("IsWindowHovered", sol::overload(sol::resolve<bool()>(IsWindowHovered),
  ...
  // Prefer  SetNext...
  imGuiTable.set_function("SetNextWindowPos",
  ...
  imGuiTable.set_function("SetNextWindowSize", sol::overload(sol::resolve<void(float, float)>(SetNextWindowSize),
  ...
  imGuiTable.set_function("SetNextWindowCollapsed",
  ...
  imGuiTable.set_function(
      "SetWindowPos",
      sol::overload(sol::resolve<void(float, float)>(SetWindowPos), sol::resolve<void(float, float, int)>(SetWindowPos),
                    sol::resolve<void(const std::string &, float, float)>(SetWindowPos),
                    sol::resolve<void(const std::string &, float, float, int)>(SetWindowPos)));
  imGuiTable.set_function("SetWindowSize",
  ...
  imGuiTable.set_function("SetWindowFocus", sol::overload(sol::resolve<void()>(SetWindowFocus),
...
void Init_WindowsScrolling(sol::table &imGuiTable) {
  ...
  imGuiTable.set_function(
      "SetScrollHereX", sol::overload(sol::resolve<void()>(SetScrollHereX), sol::resolve<void(float)>(SetScrollHereX)));
  imGuiTable.set_function(
      "SetScrollHereY", sol::overload(sol::resolve<void()>(SetScrollHereY), sol::resolve<void(float)>(SetScrollHereY)));
  imGuiTable.set_function("SetScrollFromPosX", sol::overload(sol::resolve<void(float)>(SetScrollFromPosX),
                                                             sol::resolve<void(float, float)>(SetScrollFromPosX)));
  imGuiTable.set_function("SetScrollFromPosY", sol::overload(sol::resolve<void(float)>(SetScrollFromPosY),
