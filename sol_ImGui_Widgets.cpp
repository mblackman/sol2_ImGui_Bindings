...
void Image() { /* TODO: Image(...) ==> UNSUPPORTED (ImTextureID binding) */ }

void ImageButton() { /* TODO: ImageButton(...) ==> UNSUPPORTED (ImTextureID binding) */ }
...
std::tuple<int, bool> Combo(const std::string &label, int currentItem, const sol::table &items, int itemsCount) {
  ...
  for (auto &string_item : strings) cstrings.push_back(string_item.c_str());
...
std::tuple<int, bool> Combo(const std::string &label, int currentItem, const sol::table &items, int itemsCount,
  ...
  bool clicked = ImGui::Combo(label.c_str(), &current_item_ref, cstrings.data(), itemsCount, popupMaxHeightInItems);
  return std::make_tuple(current_item_ref, clicked);
}
...
std::tuple<int, bool> Combo(const std::string &label, int currentItem, const std::string &itemsSeparatedByZeros,
                            int popupMaxHeightInItems) {
  int current_item_ref = currentItem;  // Pass by reference
  bool clicked = ImGui::Combo(label.c_str(), &current_item_ref, itemsSeparatedByZeros.c_str(), popupMaxHeightInItems);
  return std::make_tuple(current_item_ref, clicked);
}
...
std::tuple<sol::as_table_t<std::vector<float> >, bool> ColorEdit4(const std::string &label, const sol::table &col,
  ...
  const lua_Number r{col[1].get<std::optional<lua_Number> >().value_or(static_cast<lua_Number>(0))},
      g{col[2].get<std::optional<lua_Number> >().value_or(static_cast<lua_Number>(0))},
      b{col[3].get<std::optional<lua_Number> >().value_or(static_cast<lua_Number>(0))},
      a{col[4].get<std::optional<lua_Number> >().value_or(static_cast<lua_Number>(1.0f))};  // Default alpha to 1.0f
...
std::tuple<sol::as_table_t<std::vector<float> >, bool> ColorPicker4(const std::string &label, const sol::table &col) {
  return ColorPicker4(label, col, 0, nullptr);
}

std::tuple<sol::as_table_t<std::vector<float> >, bool> ColorPicker4(const std::string &label, const sol::table &col,
                                                                    int flags) {
  return ColorPicker4(label, col, flags, nullptr);
}
...
bool TreeNodeEx(const std::string &label, int flags) { return ImGui::TreeNodeEx(label.c_str(), flags); }
...
bool CollapsingHeader(const std::string &label, int flags) { return ImGui::CollapsingHeader(label.c_str(), flags); }
...
void SetNextItemOpen(bool is_open, int cond) { ImGui::SetNextItemOpen(is_open, cond); }
...
void Init_WidgetsText(sol::table &imGuiTable) {
  imGuiTable.set_function("TextUnformatted",
...
void Init_WidgetsComboBox(sol::table &imGuiTable) {
  imGuiTable.set_function("BeginCombo",
...
void Init_WidgetsColorEditorPicker(sol::table &imGuiTable) {
  ...
  imGuiTable.set_function(
      "ColorButton",
      sol::overload(sol::resolve<bool(const std::string &, const sol::table &)>(ColorButton),
...
void Init_WidgetsTrees(sol::table &imGuiTable) {
  ...
  imGuiTable.set_function("SetNextItemOpen", sol::overload(sol::resolve<void(bool)>(SetNextItemOpen),
                                                           sol::resolve<void(bool, int)>(SetNextItemOpen)));
}

void Init_WidgetsValueHelpers(sol::table &imGuiTable) {
  imGuiTable.set_function("Value",
...
void Init_FocusActivation(sol::table &imGuiTable) {
  ...
  imGuiTable.set_function("SetKeyboardFocusHere", sol::overload(sol::resolve<void()>(SetKeyboardFocusHere),
